...
var onCloudDataReceived = function(entry) {
    // Edge syncs data in the background, so this callback is very fast. It's
    // often faster than initialisation, meaning there's a race condition
    // where the UI is not always ready for the assumptions this function makes.

    function waitForWidgetToBeReady() {
        if ( uDom.nodeFromId('cloudPull') ) {
            updateWidget();
        } else {
            window.setTimeout(waitForWidgetToBeReady, 100);
        }
    }
